...
// [LOCALGC TODO] Find a better place for these
#define static_assert_no_msg(expr) static_assert(expr, #expr)

// [LOCALGC TODO] actually implement these
#define ALIGN_DOWN(a, b) a
#define ALIGN_UP(a, b) a

inline unsigned char BitScanForward(DWORD* index, uint32_t sqMask)
{
    *index = 42;
    return TRUE;
}
...
// [LOCALGC TODO] verify if we can use variadic macros
#define _snprintf_s(name, len, unused, fmt, ...) snprintf(name, len, fmt, __VA_ARGS__)
