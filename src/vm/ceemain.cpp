...
void LoadGarbageCollector()
{
    ...
    IGCHeap *pGCHeap;
    if (!igcf(gcToClr, &pGCHeap, &pGcHandleTable, &g_gc_dac_vars))
    {
        ThrowOutOfMemory();
    }

    assert(pGCHeap != nullptr);
    assert(pGcHandleTable != nullptr);
    g_pGCHeap = pGCHeap;
...
void LoadStaticGarbageCollector()
{
    ...
    if (!InitializeGarbageCollector(nullptr, &pGCHeap, &pGcHandleTable, &g_gc_dac_vars)) 
    {
        ThrowOutOfMemory();
    }
