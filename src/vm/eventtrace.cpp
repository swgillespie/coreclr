        ...
        _Inout_opt_ PVOID CallbackContext)
    {
        ...
        if (g_fEEStarted && !g_fEEShutDown)
        {
            ...
            if (ControlCode == EVENT_CONTROL_CODE_ENABLE_PROVIDER || ControlCode == EVENT_CONTROL_CODE_DISABLE_PROVIDER)
            {
                ...
                bool shouldEnable = ControlCode == EVENT_CONTROL_CODE_ENABLE_PROVIDER;
                if (bIsPublicTraceHandle)
                {
                    heap->ControlEvents(shouldEnable, 0, static_cast<GCEventLevel>(Level));
                }
                else if (bIsPrivateTraceHandle)
                {
                    heap->ControlPrivateEvents(shouldEnable, 0, static_cast<GCEventLevel>(Level));
                }
